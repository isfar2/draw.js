...
(function(){
  ...
  // create fresh canvas/overlay elements and attach them to the `.paper` container
  // This replaces any existing canvas nodes to avoid stale CSS/layout/style issues
  ...
  // create a relative container so the main canvas sits in normal flow
  // (this makes the `.paper` grow to fit the canvas height). The overlay
  // is absolutely positioned inside this container to sit on top.
  ...
  // try to match the paper's border radius so the canvas is clipped to the same rounded rect
  ...
  // style the main canvas to participate in layout
  ...
  // overlay sits above and covers the canvas
  ...
  // append new container with canvases
  container.appendChild(canvas);
  container.appendChild(overlay);
  paper.appendChild(container);
  // ensure the paper container clips any overflow from canvases
  ...
  function resizeCanvas(w = width, h = height){
    // logical requested size (in CSS pixels)
    width = w; height = h;
    ...
    // try to size the canvas to fill the visible `.paper` container so the drawable area
    // matches what the user sees. Fall back to availableW/availableH if `.paper` is missing.
    let displayW = Math.max(64, Math.min(width, availableW));
    let displayH = Math.max(64, Math.min(height, availableH));
    const paper = document.querySelector('.paper');
    // prefer sizing based on the visible canvas-area container which reflects the viewport
    const canvasArea = document.querySelector('.canvas-area');
    if(canvasArea){
      const areaRect = canvasArea.getBoundingClientRect();
      // subtract some padding so the paper fits comfortably
    ...
    // set CSS size explicitly so the displayed area matches backing buffer
    ...
    // if canvases are within the replacement container, ensure the container
    // has an explicit height so the `.paper` area expands
  ...
  // helper: return the active (topmost) layer, create one if none
  ...
  function getPointer(e){
    // Use the actual element receiving the event (overlay or canvas) so coordinates map correctly
    const el = (e.currentTarget && e.currentTarget.tagName==='CANVAS') ? e.currentTarget : canvas;
    const rect = el.getBoundingClientRect();
    // el.width/el.height are the backing buffer pixel sizes; rect.width/rect.height are CSS sizes
    const x = Math.round((e.clientX - rect.left) * (el.width / rect.width));
    const y = Math.round((e.clientY - rect.top) * (el.height / rect.height));
    // clamp to the element backing buffer to avoid coordinates outside the canvas which
    // can cause drawing/fill to address out-of-bounds pixels when the user clicks
    // outside the visible canvas area (e.g. paper padding or scroll gaps).
  ...
  // helper to enable overlay interaction when needed
  ...
  // ensure overlay pointer state follows initial tool
  ...
  // Text tool: selection + typing, fonts
  ...
  // allow adding custom fonts via FontFace API
  ...
  // Text tool implementation: selection via overlay and editable textarea
  ...
  // hook overlay pointer events for the text tool
  ...
  // color swatches: quick-pick + save
  ...
  // initialize sizes now that state exists
  resizeCanvas();
  createLayer('Layer 1');
  ...
  window.addEventListener('keydown', e=>{
    // don't trigger global shortcuts while typing in an editable element
